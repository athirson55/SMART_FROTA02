...
export function CadastroPage() {
  ...
  async function handleRegister(e) {
    e.preventDefault();
    setFeedback("");
    const t = { nome: nome.trim(), email: email.trim(), senha: senha.trim(), confirmar: confirmar.trim() };
    if (!t.nome || !t.email || !t.senha || !t.confirmar) return (setFeedback("Preencha todos os campos."), showError("Preencha todos os campos"));
    if (!isEmail(t.email))  return (setFeedback("E-mail inválido."),                showError("E-mail inválido"));
    if (t.senha !== t.confirmar) return (setFeedback("Senhas não coincidem."),      showError("Senhas não coincidem"));
    if (t.senha.length < 6) return (setFeedback("Senha deve ter ao menos 6 caracteres."), showError("Senha curta"));

    setIsLoading(true); showLoading("Criando conta...");
    try {
    ...
    <main className="register-screen">
      ...
      <section className="register-screen__form-section">
        <form className="register-card" onSubmit={handleRegister} noValidate>
          <div className="register-card__brand"><h1>SMART FROTA</h1><p>SOLUÇÕES EM FROTA</p></div>
          ...
          <label className="register-field-group" htmlFor="nome">
            ...
            <input id="nome" type="text" placeholder="Nome completo" value={nome} onChange={(e) => setNome(e.target.value)} required />
          </label>
          ...
          <p className="register-login-link"><Link to="/login">Já tem uma conta? Entrar</Link></p>
