...
const TIPOS = [
  ...
  "Troca de pneus", "Revisão geral", "Inspeção preventiva",
  "Manutenção corretiva", "Troca de correia", "Outro",
];
...
export function NovoAgendamentoModal({ open, onClose, onCreated, veiculos = [] }) {
  ...
  function validate() {
    ...
    if (form.tipo === "Outro" && !form.tipoCustom.trim()) e.tipoCustom = "Descreva o tipo";
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      onClose();
      setForm({ veiculoId: "", tipo: "", tipoCustom: "", data: today(), hora: "08:00", km: "", local: "", responsavel: "", status: "AGENDADO", observacoes: "" });
    } catch (err) {
  ...
  const footer = (
    <>
      <button type="button" className="sf-btn sf-btn-ghost" onClick={onClose} disabled={loading}>
        Cancelar
      </button>
      <button type="submit" form="form-agendamento" className="sf-btn sf-btn-primary" disabled={loading}>
    ...
    <Modal open={open} onClose={onClose} title="Novo Agendamento" size="md" footer={footer}>
      <form id="form-agendamento" onSubmit={handleSubmit} noValidate>
        <div className="sf-form-grid">
          ...
          {/* Tipo de serviço */}
          <div className="sf-field full">
            <label className="sf-label">Tipo de serviço <span className="sf-required">*</span></label>
            <select
            ...
            {errors.tipo && <span className="sf-field-error">{errors.tipo}</span>}
          </div>

          {form.tipo === "Outro" && (
            <div className="sf-field full">
              <label className="sf-label">Descreva o serviço <span className="sf-required">*</span></label>
              <input
              ...
              />
              {errors.tipoCustom && <span className="sf-field-error">{errors.tipoCustom}</span>}
            </div>
          )}
          ...
          <div className="sf-field">
            <label className="sf-label">Horário <span className="sf-required">*</span></label>
            <input
              type="time"
            ...
            />
            {errors.hora && <span className="sf-field-error">{errors.hora}</span>}
          </div>
          ...
          <div className="sf-field">
            ...
            <input
              className="sf-input" placeholder="Ex: Auto Center VH"
          ...
          <div className="sf-field">
            ...
            <input
              className="sf-input" placeholder="Ex: Dep. Técnico"
          ...
          <div className="sf-field full">
            ...
            <textarea
              className="sf-textarea" placeholder="Detalhes adicionais sobre o agendamento…"
