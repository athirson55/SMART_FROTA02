...
test.describe("reports", () => {
  ...
  test("página carrega com spinner e depois exibe conteúdo", async ({ page }) => {
    ...
    await expect(page.locator(".fg-reports-page, .fg-home-content")).toBeVisible({ timeout: 20000 });
  ...
  test("filtro de período dispara novo fetch", async ({ page }) => {
    await expect(page.locator(".filter-select").first()).toBeVisible({ timeout: 15000 });
    const periodSelect = page.locator(".filter-select").first();

    // Intercept the API call that follows the period change
    const responsePromise = page.waitForResponse(
      (res) => res.url().includes("/relatorios/completo") && res.request().method() === "GET",
      { timeout: 15000 },
    );
  ...
  test("filtro por veículo dispara novo fetch", async ({ page }) => {
    await expect(page.locator(".filter-select").nth(1)).toBeVisible({ timeout: 15000 });
  ...
  test("tabela de veículos renderiza ou exibe estado vazio", async ({ page }) => {
    await expect(
      page.locator(".table-card, .chart-empty").first(),
    ).toBeVisible({ timeout: 20000 });
  });

  test("exportar CSV: botão inicia download ou mostra toast de erro", async ({ page }) => {
    await expect(page.locator(".export-btn").first()).toBeVisible({ timeout: 20000 });
    ...
    if (download) {
      ...
      await expect(page.locator(".reports-toast.show")).toBeVisible({ timeout: 5000 });
    }
  });

  test("exportar Excel: botão verde dispara requisição ao backend", async ({ page }) => {
    await expect(page.locator(".export-btn-excel")).toBeVisible({ timeout: 20000 });
    ...
    if (download) {
      ...
      // the toast feedback
      await expect(
        page.locator(".reports-toast.show"),
  ...
  test("botão Excel desabilitado durante exportação", async ({ page }) => {
    ...
    await page.route("**/relatorios/exportar-excel**", (route) =>
      new Promise((resolve) => setTimeout(() => resolve(route.continue()), 800)),
    );
    await page.locator(".export-btn-excel").click();
    await expect(page.locator(".export-btn-excel")).toBeDisabled({ timeout: 3000 });
